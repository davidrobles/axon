...
public class DRPlot {
    ...
    public String toString() {
        ...
        for (int i = 0; i < colNames.length; i++) {
            ...
            if (i == colNames.length - 1) builder.append("\n");
            else builder.append(",");
        }

        for (int row = 0; row < rowNames.length; row++) {
            for (int col = -1; col < colNames.length; col++) {
                if (col == -1) builder.append(rowNames[row] + ",");
                else if (col == colNames.length - 1) builder.append(data[row][col] + "\n");
                else builder.append(data[row][col] + ",");
            }
        }
