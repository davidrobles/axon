...
public class ColorMap {
    ...
    /**
     * Constructs a color map for values in [0,1] the index color model. The integers 0 and 255 must
     * be valid pixels for the color model.
     *
    ...
    /**
     * Constructs a color map for specified values and index color model. The integers 0 and 255
     * must be valid pixels for the color model.
     *
    ...
    /**
     * Constructs a color map for specified values and colors. Red, green, and blue components are
     * bytes in the range [0,255], inclusive.
     *
     * @param vmin the minimum value.
     * @param vmax the maximum value.
    ...
    /**
     * Constructs a color map for specified values and colors. Red, green, and blue components are
     * floats in the range [0,1], inclusive.
     *
     * @param vmin the minimum value.
     * @param vmax the maximum value.
     * @param r array[256] of reds.
    ...
    /**
     * Sets the min-max range of values mapped to colors. Values outside this range are clipped. The
    ...
    /**
     * Gets a linear gray color model for the specified gray levels. Gray levels equal to 0.0 and
     * 1.0 correspond to colors black and white, respectively.
     *
    ...
    /**
     * Gets a linear hue color model for the specified hues. Hues equal to 0.00, 0.33, and 0.67, and
     * 1.00 correspond approximately to the colors red, green, blue, and red, respectively.
     *
    ...
    /**
     * Gets a linear hue color model for the specified hues and alpha. Hues equal to 0.00, 0.33, and
     * 0.67, and 1.00 correspond approximately to the colors red, green, blue, and red,
     * respectively.
     *
     * @param h0 the hue corresponding to index value 0.
     * @param h255 the hue corresponding to index value 255.
    ...
    public static IndexColorModel makeIndexColorModel(Color[] c) {
        if (hasAlpha(c)) {
            return new IndexColorModel(8, 256, getReds(c), getGreens(c), getBlues(c), getAlphas(c));
        } else {
    ...
    public static IndexColorModel makeSolidColors(Color c) {
        ...
        for (int i = 0; i < 256; ++i) colors[i] = c;
    ...
    private static byte[] getReds(Color[] color) {
        int n = color.length;
        byte[] r = new byte[n];
        for (int i = 0; i < n; ++i) r[i] = (byte) color[i].getRed();
        return r;
    }
    ...
    private static byte[] getGreens(Color[] color) {
        int n = color.length;
        byte[] g = new byte[n];
        for (int i = 0; i < n; ++i) g[i] = (byte) color[i].getGreen();
        return g;
    }
    ...
    private static byte[] getBlues(Color[] color) {
        int n = color.length;
        byte[] b = new byte[n];
        for (int i = 0; i < n; ++i) b[i] = (byte) color[i].getBlue();
        return b;
    }
    ...
    private static byte[] getAlphas(Color[] color) {
        int n = color.length;
        byte[] b = new byte[n];
        for (int i = 0; i < n; ++i) b[i] = (byte) color[i].getAlpha();
        return b;
    }

    private static boolean hasAlpha(Color[] color) {
        int n = color.length;
        for (int i = 0; i < n; ++i) if (color[i].getAlpha() != 255) return true;
        return false;
    }
    ...
    private static byte[] getBytes(float[] f) {
        int n = f.length;
        byte[] b = new byte[n];
        for (int i = 0; i < n; ++i) b[i] = (byte) (f[i] * 255.0f + 0.5f);
        return b;
    }
